...
// import blogData from "../../Data/blogData.json"; // Removed static data
...
const BlogGrid = () => {
  ...
  useEffect(() => {
    const fetchParcours = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/parcours');
        if (!response.ok) throw new Error('Failed to fetch');
        const data = await response.json();
        
        // Filter active parcours and map to blog card format
        // We expect 3 main parcours: Découvrir, Approfondir, Transmettre
        const formattedData = data
          .filter(p => p.is_active)
          .slice(0, 3)
          .map(p => ({
            img: p.photo || "/assets/images/blog/blog01.jpg",
            category: p.title,
            socialImg: "/assets/images/blog/social-pr01.jpg",
            name: "EnnéaMaroc",
  ...
  // Show only 3 cards
  // const items = blogData.slice(0, 3); // Replaced by state
  const solutionCards = [
    {
      category: "Managers",
